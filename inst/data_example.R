...
maindir <- "/Users/peraugustmoen/Library/CloudStorage/OneDrive-UniversitetetiOslo/project3/realdataexample"
#maindir <- "... fill in ... "
...
dat = read.csv("inst/data/exchange_rates_from_FED.csv")
...
keep_inds = c(1,3,12,5,8,2,7,21,10,19,17) # columns of date + the currencies
dat = dat[,keep_inds]
colnames(dat) = c("date", "EUR", "JPY", "GBP", "CNY", "AUD",
                  "CAD", "CHF", "HKD", "SGD", "SEK")
dat[dat=="ND"] = NA
...
dat[,2:ncol(dat)] = sweep(dat[,2:ncol(dat)], 2, as.numeric(dat[1,2:ncol(dat)]), `/`)
...
# plot the raw data
matplot(dat$date, dat[,2:ncol(dat)], type = "l", xlab = "Time",
...
plot <- plot_ly(df_long, x = ~date, y = ~value, color = ~series, type = 'scatter', mode = 'lines') %>%
  layout(
...
trainingdata = dat_diff[dat_diff$date < as.POSIXct("2001-01-01", format="%Y-%m-%d", tz = "UTC"),]
...
critical_value = MC_covariance(p=p, false_alarm_prob = 0.05,
                               constant_penalty=FALSE, MC_reps = 1000, N=1000,
                               estimate_mean=FALSE,
                               baseline_operatornorm = 1,
                               seed = 102102)

detector = CHAD(p, method = "covariance", leading_constant = critical_value,
...
plot <- plot_ly(df_long, x = ~date, y = ~value, color = ~series,
...
plot <- ggplot(df_long, aes(x = date, y = value, color = series,
                            linetype=series)) +
  geom_line() +
...
for (i in 1:length(cp_dates)) {
  plot <- plot + geom_vline(xintercept = as.numeric(cp_dates[i]), color = "red", linetype = "dashed")
}
